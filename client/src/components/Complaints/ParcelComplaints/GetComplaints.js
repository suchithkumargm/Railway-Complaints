...
const GetComplaints = (props) => {
	...
	useEffect(() => {
		...
		async function fetchComplaints() {
			try {
				const response = await fetch('https://railway-complaints-server.onrender.com/complaints/parcelcomplaints/getusercomplaints', {
					method: 'GET',
					headers: {
	...
	const handleDelete = async (complaintId) => {
		...
		try {
			await fetch(`https://railway-complaints-server.onrender.com/complaints/parcelcomplaints/deletecomplaint/${complaintId}`, {
				method: 'DELETE',
				headers: {
					'Content-Type': 'application/json',
					'authToken': localStorage.getItem('token')
