...
const NewComplaint = (props) => {
	...
	const handleSubmit = async (e) => {
		...
		const complaintData = {
			stationName: newComplaint.stationName,
			platformNumber: newComplaint.platformNumber,
		...
		try {
			const response = await fetch(
				'https://railwaycomplaints.onrender.com/complaints/stationcomplaints/newcomplaint',
				{
					method: 'POST',
					headers: {
			...
			<Container className="container">
				<Form className="w-50 login-form" onSubmit={handleSubmit}>
					<Form.Group controlId="formGridName" className="input-field">
						...
						<Form.Control type="text" placeholder="Enter Station Name" name="stationName" value={newComplaint.stationName} onChange={onChange} required />
					</Form.Group>

					<Form.Group controlId="formGridMobile" className="input-field">
						<Form.Label>Platform Number</Form.Label>
						<Form.Control type="text" placeholder="Enter platform number" name="platformNumber" value={newComplaint.platformNumber} onChange={onChange} required />
					</Form.Group>
