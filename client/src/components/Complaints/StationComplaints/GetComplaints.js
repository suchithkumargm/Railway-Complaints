...
const GetComplaints = (props) => {
	...
	useEffect(() => {
		...
		async function fetchComplaints() {
			try {
				const response = await fetch('https://railwaycomplaints.onrender.com/complaints/stationcomplaints/getusercomplaints', {
					method: 'GET',
					headers: {
    ...
    //Implement the delete logic here
		try {
			await fetch(`https://railwaycomplaints.onrender.com/complaints/stationcomplaints/deletecomplaint/${complaintId}`, {
				method: 'DELETE',
				headers: {
					'Content-Type': 'application/json',
					'authToken': localStorage.getItem('token')
