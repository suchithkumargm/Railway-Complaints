...
const GetComplaints = (props) => {
	...
	useEffect(() => {
		...
		async function fetchComplaints() {
			try {
				const response = await fetch('https://railwaycomplaints.onrender.com/complaints/traincomplaints/getusercomplaints', {
					method: 'GET',
					headers: {
    ...
    //Implement the delete logic here
		try {
			await fetch(`https://railwaycomplaints.onrender.com/complaints/traincomplaints/deletecomplaint/${complaintId}`, {
				method: 'DELETE',
				headers: {
					'Content-Type': 'application/json',
					'authToken': localStorage.getItem('token')
